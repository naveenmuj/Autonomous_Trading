...
class MarketDataWebSocket:
    ...
    def _on_close(self, wsapp, status_code, message):
        ...
        logger.info(f"WebSocket connection closed. Status: {status_code}, Message: {message}")
        self.is_connected = False

    def _on_control_message(self, wsapp, message_type, message):
            ...
            if isinstance(message, str):
                try:
                    ...
                    if data.get('type') == 'success' and data.get('message') == 'subscribed':
                        ...
                        # Attempt resubscribe on subscription errors
                        if "not subscribed" in str(data.get('message', '')).lower():
                            logger.info("Attempting to resubscribe...")
                            if self.subscribed_tokens:
                                self.subscribe(list(self.subscribed_tokens.keys()))
                                
                except json.JSONDecodeError:
                    logger.warning(f"Failed to parse control message as JSON: {message}")
                    
        except Exception as e:
            logger.error(f"Error processing control message: {str(e)}")
            if message:
                logger.debug(f"Raw message at error: {message}")
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            self.ws.on_control_message = self._on_control_message
    ...
    def unsubscribe(self, tokens: List[str]) -> None:
        ...
        try:
            ...
            self.ws.unsubscribe(correlation_id=correlation_id, token_list=token_list)
